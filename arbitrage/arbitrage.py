...
class ArbitrerCLI:
    ...
    def get_broker_balance(self, args):
        ...
        while True:
            ...
            if accounts:
                ...
                cny_total = (btc_balance + btc_frozen) * \
                    ticker.bid + cny_balance + cny_frozen
                btc_total = btc_balance + btc_frozen + \
                    (cny_balance + cny_frozen) / ticker.bid
                ...
                broker_msg += "%s:\t\t %s\n" % ('Profit', str(
                    {"profit": '%.2fCNY / %.2fBTC' % (cny_diff + cny_bonus, btc_bonus + btc_diff)}))
    ...
    def init_logger(self, args):
        ...
        Rthandler = RotatingFileHandler(
            'arbitrage.log', maxBytes=100 * 1024 * 1024, backupCount=10)
