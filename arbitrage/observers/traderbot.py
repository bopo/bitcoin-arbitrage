...
class TraderBot(BasicBot):
    ...
    def check_order(self, depths):
        ...
        if self.is_buying():
            ...
            for buy_order in buy_orders:
                ...
                result = self.clients[buy_order['market']
                                      ].get_order(buy_order['id'])
                ...
                if result['status'] == 'CLOSE' or result['status'] == 'CANCELED':
                    if result['status'] == 'CANCELED':
                        ...
                        self.clients[self.hedger].buy(
                            left_amount, result['price'])

                    self.remove_order(buy_order['id'])
                else:
                    try:
                        ask_price = int(
                            depths[buy_order['market']]["asks"][0]['price'])
                    ...
                    if abs(result['price'] - ask_price) > config.arbitrage_cancel_price_diff:
                        ...
                        self.cancel_order(
                            buy_order['market'], 'buy', buy_order['id'])
        ...
        if self.is_selling():
            ...
            for sell_order in self.get_orders('sell'):
                ...
                result = self.clients[sell_order['market']
                                      ].get_order(sell_order['id'])
                logging.debug(result)
                if not result:
                ...
                if result['status'] == 'CLOSE' or result['status'] == 'CANCELED':
                    ...
                    try:
                        bid_price = int(
                            depths[sell_order['market']]["bids"][0]['price'])
                    except Exception as ex:
                        logging.warn("exception depths:%s" % ex)
                        traceback.print_exc()
                        continue
                    ...
                    if abs(result['price'] - bid_price) > config.arbitrage_cancel_price_diff:
                        ...
                        self.cancel_order(
                            sell_order['market'], 'sell', sell_order['id'])
        ...
        if profit < self.reverse_profit_thresh and perc < self.reverse_perc_thresh:
            ...
            if self.clients[kbid].btc_balance < self.stage0_percent * self.init_btc[kbid] or self.clients[kbid].cny_balance < self.stage0_percent * self.init_cny[kbid]:
                ...
                logging.info("%s %s btc:%s < %s,cny:%s < %s,  reverse", self.stage0_percent, kbid,
                             self.clients[kbid].btc_balance,  self.stage0_percent * self.init_btc[kbid], self.clients[kbid].cny_balance, self.stage0_percent * self.init_cny[kbid])
                ktemp = kbid
                kbid = kask
                kask = ktemp
                ...
                logging.info("%s %s btc:%s < %s, cny:%s <%s, go on", self.stage1_percent, kask,
                             self.clients[kask].btc_balance, self.stage1_percent * self.init_btc[kask], self.clients[kask].cny_balance, self.stage1_percent * self.init_cny[kask])
            else:
    ...
    def execute_trade(self, volume, kask, kbid, weighted_buyprice,
        ...
        logging.info(
            "trend is %s[%s->%s]", "up, buy then sell" if self.trend_up else "down, sell then buy", self.last_bid_price, buyprice)
